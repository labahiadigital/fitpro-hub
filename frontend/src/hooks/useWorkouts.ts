...
const demoExercises: Exercise[] = [
  { id: 'ex-1', name: 'Press de Banca', muscle_groups: ['pecho', 'tríceps'], equipment: ['barra', 'banco'], difficulty: 'intermediate', description: 'Ejercicio compuesto para pecho' },
  { id: 'ex-2', name: 'Sentadilla', muscle_groups: ['cuádriceps', 'glúteos'], equipment: ['barra'], difficulty: 'intermediate', description: 'Ejercicio fundamental para piernas' },
  { id: 'ex-3', name: 'Peso Muerto', muscle_groups: ['espalda', 'isquiotibiales'], equipment: ['barra'], difficulty: 'advanced', description: 'Ejercicio compuesto para espalda baja' },
  { id: 'ex-4', name: 'Dominadas', muscle_groups: ['espalda', 'bíceps'], equipment: ['barra de dominadas'], difficulty: 'intermediate', description: 'Ejercicio de tracción vertical' },
  { id: 'ex-5', name: 'Press Militar', muscle_groups: ['hombros', 'tríceps'], equipment: ['barra'], difficulty: 'intermediate', description: 'Ejercicio para hombros' },
  { id: 'ex-6', name: 'Curl de Bíceps', muscle_groups: ['bíceps'], equipment: ['mancuernas'], difficulty: 'beginner', description: 'Aislamiento para bíceps' },
  { id: 'ex-7', name: 'Extensión de Tríceps', muscle_groups: ['tríceps'], equipment: ['mancuernas'], difficulty: 'beginner', description: 'Aislamiento para tríceps' },
  { id: 'ex-8', name: 'Zancadas', muscle_groups: ['cuádriceps', 'glúteos'], equipment: ['mancuernas'], difficulty: 'beginner', description: 'Ejercicio unilateral para piernas' },
  { id: 'ex-9', name: 'Plancha', muscle_groups: ['core'], equipment: ['ninguno'], difficulty: 'beginner', description: 'Ejercicio isométrico para core' },
  { id: 'ex-10', name: 'Remo con Barra', muscle_groups: ['espalda', 'bíceps'], equipment: ['barra'], difficulty: 'intermediate', description: 'Ejercicio de tracción horizontal' },
]
...
const demoPrograms: WorkoutProgram[] = [
  ...
  },
  {
    ...
    workspace_id: '11111111-1111-1111-1111-111111111111',
    name: 'Fuerza Básica',
    description: 'Programa para principiantes enfocado en los movimientos básicos',
    duration_weeks: 4,
    difficulty: 'beginner',
    template: { weeks: [] },
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      notifications.show({
        ...
        message: isDemoMode ? 'Ejercicio creado (modo demo)' : 'El ejercicio ha sido creado correctamente',
        color: 'green',
      })
    },
    onError: (error: Error) => {
    ...
    queryFn: async () => {
      if (isDemoMode) {
        ...
        if (isTemplate !== undefined) {
          filteredPrograms = filteredPrograms.filter(p => p.is_template === isTemplate)
        }
        return { data: filteredPrograms }
      }
      return workoutsApi.programs({ is_template: isTemplate ? 'Y' : undefined })
    },
...
export function useCreateWorkoutProgram() {
  ...
  return useMutation({
    mutationFn: async (data: Partial<WorkoutProgram>) => {
      if (isDemoMode) {
        return { data: { ...data, id: `demo-prog-${Date.now()}`, created_at: new Date().toISOString() } }
      }
    ...
    onSuccess: () => {
      ...
      notifications.show({
        ...
        message: isDemoMode ? 'Programa creado (modo demo)' : 'El programa de entrenamiento ha sido creado correctamente',
        color: 'green',
      })
    },
    onError: (error: Error) => {
