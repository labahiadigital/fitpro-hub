...
export function useAuth() {
  ...
  const { setUser, setTokens, setWorkspace, logout: clearStore } = useAuthStore()
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (data.session) {
        setTokens(data.session.access_token, data.session.refresh_token || undefined)
        ...
        try {
          ...
          console.log('User not found in DB, will be created on first workspace access')
        }
