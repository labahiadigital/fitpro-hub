...
export function useCreateAppointment() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      notifications.show({ title: i18next.t("hooks.appointmentCreated"), message: i18next.t("hooks.appointmentBooked"), color: "green" });
    },
...
export function useUpdateAppointment() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["appointments"] });
      notifications.show({ title: i18next.t("hooks.appointmentUpdated"), message: i18next.t("hooks.changesSaved"), color: "green" });
    },
...
export function useUpdateAppointmentStatus() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["appointments"] });
      notifications.show({ title: i18next.t("hooks.statusUpdated"), message: i18next.t("hooks.appointmentStatusUpdated"), color: "green" });
    },
    onError: (e: unknown) => notifications.show({ title: "Error", message: getApiErrorMessage(e), color: "red" }),
  });
}
...
export function useDeleteAppointment() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["appointments"] });
      notifications.show({ title: i18next.t("hooks.appointmentDeleted"), message: i18next.t("hooks.appointmentHasBeenDeleted"), color: "green" });
    },
  });
}
