                ...
                {payments.slice(0, 5).map((payment) => {
                  ...
                  return (
                    <Group
                      ...
                      key={payment.id}
                      p="sm"
                      style={{ borderRadius: "var(--radius-item)", transition: "background 0.2s", cursor: "pointer" }}
                      className="hover-lift"
                    >
                      <Group gap="sm">
                        <ThemeIcon color={getStatusColor(payment.status)} radius="xl" size="lg" variant="light">
                          <PaymentIcon size={16} />
                        </ThemeIcon>
                        <Box>
                ...
                {payments.map((payment) => {
                  ...
                  return (
                    <Table.Tr key={payment.id} style={{ transition: "background 0.2s" }}>
                      ...
                      <Table.Td>
                        <Badge color={getStatusColor(payment.status)} variant="light" radius="xl">{getStatusLabel(payment.status)}</Badge>
                      </Table.Td>
                      <Table.Td>
              ...
              {invoiceForm.values.items.map((item, idx) => (
                <Box key={idx} className="nv-card-compact" p="sm" style={{ borderRadius: "var(--radius-sm)", border: "1px solid var(--nv-border)" }}>
                  <Group align="flex-start" gap="sm" wrap="wrap">
                    ...
                    />
                    <NumberInput label={idx === 0 ? "Dto.%" : undefined} min={0} max={100} suffix="%" style={{ flex: 0.6, minWidth: 70 }} {...invoiceForm.getInputProps(`items.${idx}.discount_value`)} />
                    ...
                    {invoiceForm.values.items.length > 1 && (
                      <ActionIcon color="red" variant="subtle" radius="xl" mt={idx === 0 ? 24 : 0} onClick={() => invoiceForm.removeListItem("items", idx)}>
                        <IconTrash size={14} />
                      </ActionIcon>
                    )}
                  </Group>
          ...
          <ScrollArea.Autosize mah="75vh">
            <Stack gap="md" pr="xs">
                          ...
                          <Box p="md" style={{ borderRadius: "var(--radius-sm)", border: "2px dashed var(--nv-border)" }}>
                            ...
                            >
                              <Group justify="center" gap="xl" mih={80} style={{ pointerEvents: "none" }}>
                                <Dropzone.Accept><IconUpload size={40} stroke={1.5} color="var(--nv-primary)" /></Dropzone.Accept>
              ...
              <Box className="nv-card-compact" p="md" style={{ borderRadius: "var(--radius-sm)", border: "1px solid var(--nv-border)" }}>
                ...
                {verifactuTestResult && (
                  <Stack gap="xs" mt="md">
                    <Badge size="lg" color={verifactuTestResult.success ? "green" : "red"} variant="light">
                    ...
                    {verifactuTestResult.checks.map((check: { ok: boolean; detail: string }, i: number) => (
                      <Group key={i} gap="xs">
                        <ThemeIcon size="xs" radius="xl" color={check.ok ? "green" : "red"} variant="filled">
